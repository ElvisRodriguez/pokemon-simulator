...
class Pokemon:
    ...
    def effort_values(self, stat: Stat, effort_value_points: int) -> None:
        self._effort_values[stat] += effort_value_points
    ...
    def experience(self, _experience: int) -> None:
        if self._level < MAX_LEVEL:
            ...
            next_level_experience = calculations.get_experience(self._experience_group, self._level + 1)
            while next_level_experience < self._experience:
                self._level += 1
                next_level_experience = calculations.get_experience(self._experience_group, self._level)
